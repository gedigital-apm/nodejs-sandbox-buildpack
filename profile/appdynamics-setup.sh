...
then
  ...
  APPDYNAMICS_AGENT_ACCOUNT_ACCESS_KEY=$(echo "${VCAP_SERVICES-}" | jq -r '.["user-provided"][] | select(.name=='\""$key"\"') | .credentials | .["account-access-key"] ')
  APPDYNAMICS_AGENT_APPLICATION_NAME=$(echo "${VCAP_SERVICES-}" | jq -r '.["user-provided"][] | select(.name=='\""$key"\"') | .credentials | .["application-name"] ')
